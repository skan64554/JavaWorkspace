...
import com.kh.chap01_thread.model.thread.Thread_a;
import com.kh.chap01_thread.model.thread.Thread_b;
...
public class Run{
	public static void main(String[] args) {
		 ...
		 * 
		 * 3) cpu를 효율적으로 사용 가능
		 *  - 멀티스레드환경에서는 여러 작업이 동시에 수행되므로, cpu의 대기 시간이 줄어든다.
		 *  
		 *  멀티스레드 프로그래밍시 주의점
		 *  - 동기화 처리가 복잡해진다.
		...
		// th1.start(); // 스레드 생성
		 // 메인 스레드 생성 -> stack 메모리 할당 -> main()메서드 실행
		  ...
		  *    실제로는 OS의 "스케쥴러"에 의해 스레드의 전환이 순식간에 어루어져 동시에 실행되는 것처럼 보이는 현상
		  ...
		  *  - 메인 스레드가 종료되더라도, 개별 스레드가 실행중이라면 프로세스가 종료되지 않는다
		  */
		...
		*/Thread tha = new Thread(() -> {
			...
			for(int i = 97; i <= 122; i++ ) {
				System.out.println("[소문자스레드 : " + (char)i + "]");
			}
			System.out.println("[소문자스레드 종료]");
		 });
		 Thread thA = new Thread(() -> {
				System.out.println("[대문자스레드 시작]");
				for(int i = 65; i <= 90; i++ ) {
					System.out.println("[대문자스레드 : " + (char)i + "]");
				}
				System.out.println("[대문자스레드 종료]");
			 });
		 tha.start();
		 thA.start();
	}
