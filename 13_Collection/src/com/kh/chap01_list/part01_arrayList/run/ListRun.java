...
public class ListRun {
	 ...
	 * - 효율적인 기능들이 정의되어있는 틀
	 * 
	 * 데이터들이 새롭게 추가되거나 삭제가되거나, 수정이 되는 기능들을 이미 정의해놓은 틀
	 * 
	 * 배열과 컬렉션(List)의 차이점
	 * - 배열의 단점
	 * 1. 같은 자료형인 데이터만 저장가능
	 * 2. 배열의 크기를 수정할 수 없다.
	 * 3. 배열내부의 갈등을 조작하기 불편하다.
	 * 
	 * - 컬렉션의 장점
	 * 1. 여러자료형의 값을 저장할 수 있다.(제네릭 설정을 통해 한가지유형만 저장도 가능)
	 * 2. 중간의 값을 추가하거나, 삭제하는등 배열내부의 값들을 조작하기위한 편리한 메서드가 정의가 됨
	 ...
	 * 
	 * Map계열
	 * - 담고자 하는 값(value)과 그 값을 저장하는 키(key)를 함께 저장한다
	 * - 값 저장시 순서를 유지하지 않는다
	 * - 키(key)값의 중복은 허용하지 않는다. value 중복은 허용
	 * - HashMap, Properties...
	 */
	
		public static void main(String[] args) {
			//ArrayList list = new ArrayList(); // 크기 10배열 생성
			
			/*
			 * 제네릭 ( <E> )
			 * - 별도의 제네릭 설정없이 컬렉션객체 생성시 디폴트 : Object
			 * - 제네릭을 설정하면 설정한 탑의 값들만을 컬렉션에 담을 수 있다
			 * 
			 * 제네릭을 설정하는 이유
			 * 1. 명시한 타입의 객체만 저장가능하도록 하여 "타입의 안전성"을 확보하기 위해서
			 * 2. 컬렉션에 저장된 객체를 꺼내서 사용할 때 직접 형변환하는 작업을 없애기 위해서
			 */
			
			// 컬렉션에는 Music타입의 객체만 저장 가능
			// [null,null,null]?
			ArrayList<Music> list = new ArrayList<>(3); // 객체 생성시에는 제네릭 생성가능
			
			System.out.println(list); // 컬렉션 size기준 내용 출력
			...
			//2. add(int index, E e) : 리스트에 전달되는 index 위치에 e를 추가하는 메서드
			...
			System.out.println("===========================================");
			// 13. sort(List list) : 배열을 정렬해주는 메서드
			 ...
			 *  - vo클래스 이외 별도 클래스에 Comparator를 상속시켜서 구현
			 *  - 여러개의 정려조건을 만들 수 있다
			 */
			System.out.println("===========================================");
