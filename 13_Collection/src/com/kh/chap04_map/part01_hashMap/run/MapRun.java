...
public class MapRun {
	...
	public static void main(String[] args) {
		 ...
		 *  - 계층구조상 List, Set계열과 다른 Map 계열 인터페이스를 상속한 클래스로,
		 *  사용하는 메서드가 다르다.
		 *  - 데이터를 저장할 때 key, value를 한 쌍으로 저장하고, key를 통해 value가 저장된 위치에 접근
		 *  - key값의 중복을 허용하지 않는다
		 *  - 동일한 key값에 데이터를 저장하면, 마지막으로 추가된 value값이 저장된다 
		 */
		// key : String, value : Snack
		...
		// 5. remove(Key K) : KEY값과 일치하는 값을 찾아서 제거해주는 메서드( key와 value 함께 제거 )
		hm.remove("다이제");
