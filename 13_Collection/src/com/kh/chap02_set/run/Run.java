...
public class Run {
	public static void main(String[] args) {
		 ...
		 *  TreeSet : 중복을 허용하지 않지만 오름차순정렬을 지원한다
		 */
		 ...
		 * 1. 객체의 hashCode()로 객체를 저장할 위치(주소값)를 찾는다.
		 * 2. 저장한 위치에 이미 저장된 값이 있다면 equals()를 통해 저장된 값과
		 * 새로 추가할 값이 같은지 검사한다
		 * 3. 두 값이 같다면 데이터를 중복으로 저장하지 않고, 두 값이 다르다면 
		 * 저장된 같의 다음위치에 새로운 값을 저장한다.
		 */
		Set<Student> hs2 = new HashSet<>();
		hs2.add(new Student("공유",40,100));
		hs2.add(new Student("김현민",36,1));
		hs2.add(new Student("국도레시",36,100));
		hs2.add(new Student("공유",40,100));
		
		System.out.println(hs2);
		System.out.println(hs2.size()); // 4 주소값이 달라서 동일한것으로 판단, hashCode 변경후 -> 3
		
		Student s1 = new Student("김현민",36,1);
		Student s2 = new Student("김현민",36,1);
		...
		Set<Integer> set2 = new TreeSet<>(comp); // 내림차순 정렬조건은 가진 TreeSet
		set2.addAll(set1);
