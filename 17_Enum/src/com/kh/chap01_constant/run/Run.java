...
public class Run {
	public static void main(String[] args) {
		 ...
		 *  - static final 키워드를 사용하여 static 영역에 데이터를 저장하고 재사용
		...
		// 그 외 상수의 단점
		/*
		 * - 비슷한 상수값을 하나의 변수로 관리할 수 있음
		 * EX) 다양한 애플리케이션에서 사용되는 상수값이 있을떄, 이를 하나의 변수로 관리하고 싶은 경우
		 * 	   JAVA에서는 boolean으로 관리하는 값을, Oracle에서는 true라는 문자열로 관리한다고 할때
		 * 	   두 값은 하나의 묶음으로 처리해야 한다
		 * - 상수마다 고유의 메서드를 사용할 수 없다.
		 * 
		 * 상수의 단점을 보완한 코딩바식
		 * 	- 값 자체에 의미를 부여, 검증기능을 추가하고, 고유 메서드를 추가하고, 비슷한 값들을 하나로 모아서 관리
		 *  - 상수를 객체로 보관
		 */
		Car car2 = new Car();
