...
public class LambdaRun {
	 ...
	 *  - 자바에서 함수형 프로그래밍을 지원하기 위해 JDK 8버전부터 추가도니 기능.
	...
	public static void main(String[] args) {
		...
		//람다식
		/*
		 *  LambdaTest1인터페이스에 들어갈 수 있는 객체는 무조건 LambdaTest1 인터페이스를 
		 *  상속받아 추상메서드를 구현한 클래스여야 한다.
		 *  
		 *  익명 내부 클래스의 문법처럼 이름없는 클래스의 형태로 객체를 생성해야 하지만,
		 *  불필요한 코드를 줄이는 방식으로 간소화 되었다.
		 *  
		 *  함수형 인터페이스는 고정적으로 1개의 추상메서드만 보관하고 있으므로
		 *  구현해야할 함수의 이름, 반환형 매개변수의 자료형등은 생략이 가능하다
		 */
