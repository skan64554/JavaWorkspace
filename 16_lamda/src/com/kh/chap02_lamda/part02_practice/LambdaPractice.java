...
public class LambdaPractice {
	...
	public static void main(String[] args) {
		...
		LambdaTest2 a4 = (str) -> str.toUpperCase();
		
        // 5. (int x , int y) -> { return (x * y); };
		LambdaTest1 a5 = (x , y) -> x * y;
		...
		// 답 : (x,str1) -> System.outprintln(x+str1)
		
        // 9. (int[] arr) -> { return (Math.max(Math.max(arr[0], arr[1]), arr[2])); };
		// 답 : (arr) -> return (Math.max(Math.max(arr[0], arr[1]), arr[2]));
		
        // 10. ((String str1, String str2) -> { String result =  (str1.conact(str2)); return result; });
		// 답 : (str1, str2) -> { String result  str1.concact(str2)); return result; };
