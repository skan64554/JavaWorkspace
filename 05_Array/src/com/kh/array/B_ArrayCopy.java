...
public class B_ArrayCopy {
	...
	public void method1() {
		 ...
		 * 배열복사 
		 * - 얕은 복사 : 배열의 주소값만을 복사
		 * - 깊은 복사 : 동일한 새로운 배열을 하나 생성하여 내부의 값까지 복사
		 */
	...
	public void method2() {
		// 깊은 복사 4가지
		// 1. for문을 활용한 복사
		// 	  - 새로운 배열 생성 후 각 인덱스 별로 내부값을 일일히 대입
		int [] origin = {1,2,3,4,5};
	...
	public void method3() {
		// 2. 새로운 배열 생성 후 System 클래스의 arraycopy를 이용해 복사 (효율적)
		int [] origin = {1,2,3,4,5};
		int [] copy = new int [10];
	...
	public void method4() {
		...
		int[] copy = Arrays.copyOf(origin, 3); // 복사할 갯수 == 복사본 배열의 크기
	...
	public void method5() {
		...
		int[] copy = origin.clone();// 인덱스 지정, 복사 갯수 지정 X
		System.out.println(Arrays.toString(copy));
